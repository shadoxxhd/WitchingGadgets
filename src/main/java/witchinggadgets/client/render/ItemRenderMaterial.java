...
public class ItemRenderMaterial implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack stack, Object... data) {
                ...
                switch (scan.type) {
                    ...
                    case 2:
                        if ((scan.entity instanceof EntityItem)) {
                            scanStack = new ItemStack(
                                    ((EntityItem) scan.entity).getEntityItem().getItem(),
                                    1,
                                    ((EntityItem) scan.entity).getEntityItem().getItemDamage());
                            scanAspects = ThaumcraftCraftingManager.getObjectTags(scanStack);
                            scanAspects = ThaumcraftCraftingManager.getBonusTags(scanStack, scanAspects);
                        } else {
        ...
        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON) {
            ...
            int i = mc.theWorld.getLightBrightnessForSkyBlocks(
        ...
        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON) {
            GL11.glColor4f(
                    (aspectColour >> 16 & 255) / 255f,
                    (aspectColour >> 8 & 255) / 255f,
                    (aspectColour & 255) / 255f,
        ...
        if (scanStack != null) {
            ...
            if (type == ItemRenderType.EQUIPPED_FIRST_PERSON) GL11.glScalef(20, 20, 20);
            else {
            ...
            if (scanEntity instanceof IBossDisplayData) {
                ...
                if (BossStatus.bossName.equals(boss.func_145748_c_().getFormattedText())
                        && BossStatus.healthScale == (boss.getHealth() / boss.getMaxHealth()))
                    if (BossStatus.statusBarTime > 0) BossStatus.statusBarTime = 0;
            }
        }
