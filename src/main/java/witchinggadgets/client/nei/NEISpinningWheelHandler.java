...
public class NEISpinningWheelHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId == getOverlayIdentifier()) {
            for (SpinningRecipe recipe : SpinningRecipe.recipeList)
                if (recipe != null && recipe.getOutput() != null) arecipes.add(new CachedSpinningWheelRecipe(recipe));
        } else {
    ...
    public void loadCraftingRecipes(ItemStack result) {
        ...
        if (recipe != null) this.arecipes.add(new CachedSpinningWheelRecipe(recipe));
    }
