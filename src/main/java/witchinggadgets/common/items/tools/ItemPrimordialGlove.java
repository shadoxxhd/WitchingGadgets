...
public class ItemPrimordialGlove extends Item implements IPrimordialCrafting {
    ...
    public static ItemStack[] getSetGems(ItemStack bracelet) {
        ...
        if ((!(bracelet.getItem() instanceof ItemPrimordialGlove)) || bracelet.getTagCompound() == null) return gems;
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (!world.isRemote && tile != null && tile instanceof INode) {
            ...
            for (Aspect aspect : temp.getAspects()) {
                ...
                if (node.getNodeModifier() == NodeModifier.BRIGHT) amt = (int) (amt * 1.2F);
                if (node.getNodeModifier() == NodeModifier.PALE) amt = (int) (amt * 0.8F);
                if (node.getNodeModifier() == NodeModifier.FADING) amt = (int) (amt * 0.5F);
                amt = MathHelper.floor_double(MathHelper.sqrt_double(amt));
                if (node.getNodeType() == NodeType.UNSTABLE) amt += world.rand.nextInt(5) - 2;
                if (amt >= 1) primals.merge(aspect, amt);
            }
    ...
    public ItemStack setContainedNode(ItemStack bracelet, NodeType type, NodeModifier modifier, AspectList aspects) {
        if (!bracelet.hasTagCompound()) bracelet.setTagCompound(new NBTTagCompound());

        NBTTagCompound nodeTag = new NBTTagCompound();
        nodeTag.setInteger("type", type.ordinal());
        if (modifier != null) nodeTag.setInteger("modifier", modifier.ordinal());
