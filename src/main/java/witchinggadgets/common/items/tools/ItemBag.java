...
public class ItemBag extends Item {
    ...
    public void removeColorFromItemStack(ItemStack stack) {
        ...
        if (tag != null) {
            ...
            if (tag1.hasKey("color")) tag1.removeTag("color");
        }
    }
    ...
    public void modifyColorOnItemStack(ItemStack stack, int par2) {
        ...
        if (!tag.hasKey("display")) tag.setTag("display", tag1);
    ...
    public void registerIcons(IIconRegister iconRegister) {
        ...
        for (int i = 0; i < overlayIcons.length; i++)
            if (i != 0) overlayIcons[i] = iconRegister.registerIcon("witchinggadgets:bagOverlay_" + subNames[i]);
    }
