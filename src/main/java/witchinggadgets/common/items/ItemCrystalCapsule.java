...
public class ItemCrystalCapsule extends Item implements IFluidContainerItem {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (!world.isRemote) {
            ...
            if (tryFillTank(stack, world, x, y, z, localMovingObjectPosition.sideHit, player)) return stack;
            ...
            if (!player.inventory.addItemStackToInventory(used)) player.dropPlayerItemWithRandomChoice(used, false);
        }
        return stack;
    }
    ...
    boolean tryFillTank(ItemStack stack, World world, int x, int y, int z, int side, EntityPlayer player) {
        ...
        if (te instanceof IFluidHandler) {
            ...
            if (fs == null) {
                ...
                if (input == null || input.amount < 1000) return false;
    ...
    ItemStack useCapsule(World world, int x, int y, int z, ItemStack stack) {
        if (this.getFluidStored(stack) != null) {
            ...
            if (f == null || f.getBlock() == null || !f.canBePlacedInWorld()) return stack;
            ...
            if (!filled.hasTagCompound()) filled.setTagCompound(new NBTTagCompound());
    ...
    public int fill(ItemStack container, FluidStack resource, boolean doFill) {
        if (resource == null || resource.amount < 1000) return 0;
        if (!container.hasTagCompound()) container.setTagCompound(new NBTTagCompound());
        if (doFill) container.getTagCompound().setString("fluid", FluidRegistry.getFluidName(resource));
        return 1000;
    }
    ...
    public FluidStack drain(ItemStack container, int maxDrain, boolean doDrain) {
        ...
        Fluid f = this.getFluidStored(container);
        if (f == null) return null;
        if (doDrain) container.getTagCompound().removeTag("fluid");
        return new FluidStack(f, 1000);
    }
    ...
    public static class CapsuleDispenserBehaviour extends BehaviorDefaultDispenseItem {
        ...
        public ItemStack dispenseStack(IBlockSource dispenser, ItemStack stack) {
            ...
            ItemStack s = capsule.useCapsule(
                    dispenser.getWorld(),
