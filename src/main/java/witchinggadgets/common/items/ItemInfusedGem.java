...
public class ItemInfusedGem extends Item implements IInfusedGem {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (getCut(stack) == GemCut.POINT) {
            if (!this.performEffect(GemCut.POINT.toString(), getAspect(stack), potency, brittle, player)) return stack;
        ...
        if (cut == GemCut.POINT.toString()) {
            ...
            if (aspect.equals(Aspect.FIRE)) {
                ...
                if (world.isAirBlock(x, y + 2, z) && world.getBlockLightValue(x, y + 2, z) < 10) {
                    if (!world.isRemote) world.setBlock(x, y + 2, z, WGContent.BlockCustomAiry);
        ...
        if (cut == GemCut.OVAL.toString()) {
            ...
            if (aspect.equals(Aspect.EARTH)) {
                if (!Loader.isModLoaded("gregtech") && mop != null
                    ...
                    if (world.isRemote)
                        for (ChunkCoordinates cc : ores) ClientTickHandler.oreHighlightMap.put(cc, 1000);
                    dmg = true;
                } else {
                    if (!world.isRemote) {
            ...
            if (aspect.equals(Aspect.ORDER)) {
                ...
                if (TileNode.locations.get(node) != null) {
                    ...
                    if (world.isRemote) {
                        AspectList al = world.getTileEntity(nX, nY, nZ) instanceof TileNode
                                ? ((TileNode) world.getTileEntity(nX, nY, nZ)).getAspects()
                                : new AspectList();
                        int col = 0xffffff;
                ...
                if (Config.allowMirrors && !Loader.isModLoaded("dreamcraft")) {
                    if (mop != null && mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                        ...
                        if (tile instanceof TileMirror || tile instanceof TileMirrorEssentia) {
                            boolean link = tile instanceof TileMirror
                                    ? ((TileMirror) world.getTileEntity(targetX, targetY, targetZ)).linked
                                    : ((TileMirrorEssentia) world.getTileEntity(targetX, targetY, targetZ)).linked;
                            if (link) {
                                int dim = tile instanceof TileMirror
                                        ? ((TileMirror) world.getTileEntity(targetX, targetY, targetZ)).linkDim
                                        : ((TileMirrorEssentia) world.getTileEntity(targetX, targetY, targetZ)).linkDim;
                                int tx = tile instanceof TileMirror
                                        ? ((TileMirror) world.getTileEntity(targetX, targetY, targetZ)).linkX
                                        : ((TileMirrorEssentia) world.getTileEntity(targetX, targetY, targetZ)).linkX;
                                int ty = tile instanceof TileMirror
                                        ? ((TileMirror) world.getTileEntity(targetX, targetY, targetZ)).linkY
                                        : ((TileMirrorEssentia) world.getTileEntity(targetX, targetY, targetZ)).linkY;
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        return super.getItemStackDisplayName(stack)
                + (getAspect(stack) != null ? " (" + getAspect(stack).getName() + ")" : "");
    }
    ...
    public IIcon getIconIndex(ItemStack stack) {
        if (getCut(stack) != null) return icons[getCut(stack).ordinal()];
        return icons[0];
    }
    ...
    public static ItemStack createGem(Aspect asp, ItemInfusedGem.GemCut cut, boolean forceCreate) {
        if (!forceCreate && (asp == null || cut == null)) return null;
        ItemStack stack = new ItemStack(WGContent.ItemInfusedGem);
        stack.setTagCompound(new NBTTagCompound());
        if (cut != null) stack.getTagCompound().setByte("GemCut", (byte) cut.ordinal());
        if (asp != null) stack.getTagCompound().setString("Aspect", asp.getTag());
        return stack;
    }
    ...
    public static ItemInfusedGem.GemCut getCut(ItemStack stack) {
        if (!stack.hasTagCompound() || !stack.getTagCompound().hasKey("GemCut")) return null;
