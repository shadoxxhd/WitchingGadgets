...
public class ItemMaterials extends Item {
    ...
    public int getColorFromItemStack(ItemStack stack, int par2) {
        if (stack.getItemDamage() == 3) return Aspect.VOID.getColor();
        else if (stack.getItemDamage() == 4) return Aspect.GREED.getColor();
        else if (stack.getItemDamage() == 5) return Aspect.MAGIC.getColor();
        else if (stack.getItemDamage() == 99) return Color.HSBtoRGB(0.75F, 0.5F, 1F);
        else return super.getColorFromItemStack(stack, par2);
    }
    ...
    public IIcon getIconFromDamage(int metadata) {
        if (metadata < icon.length) return icon[metadata];
        return icon[0];
    }
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int targetX, int targetY,
        ...
        if (subNames[stack.getItemDamage()] == "calculator" && !world.isRemote) {
            ...
            if (world.getTileEntity(targetX, targetY, targetZ) instanceof TileInfusionMatrix) {
                ...
                player.addChatMessage(
                        new ChatComponentTranslation(Lib.CHAT + "infusionInfo.stabilityTotal", (symmetry * -1))
