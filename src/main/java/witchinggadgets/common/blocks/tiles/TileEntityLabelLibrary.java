...
public class TileEntityLabelLibrary extends TileEntityWGBase implements IInventory {
    ...
    public void updateEntity() {
        if (!this.worldObj.isRemote) this.inventory[1] = this.getOutput();
    }
    ...
    public void readCustomNBT(NBTTagCompound tag) {
        ...
        for (int i = 0; i < tagList.tagCount(); i++) {
            ...
            if (slot >= 0 && slot < inventory.length) inventory[slot] = ItemStack.loadItemStackFromNBT(itemTag);
        }
        if (tag.hasKey("aspect")) aspect = Aspect.getAspect(tag.getString("aspect"));
    ...
    public void writeCustomNBT(NBTTagCompound tag) {
        ...
        if (aspect != null) tag.setString("aspect", aspect.getTag());
