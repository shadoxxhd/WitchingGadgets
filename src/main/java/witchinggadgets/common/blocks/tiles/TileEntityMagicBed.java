...
public class TileEntityMagicBed extends TileEntity {
    ...
    public boolean activate(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY,
            float hitZ) {
    ...
    public static ChunkCoordinates getNearestEmptyChunkCoordinates(World par0World, int par1, int par2, int par3,
        ...
        for (int k1 = 0; k1 <= 1; ++k1) {
            ...
            // {
            // if (par0World.doesBlockHaveSolidTopSurface(l2, par2 - 1, i3) && !par0World.getBlockMaterial(l2, par2,
            // i3).isOpaque() && !par0World.getBlockMaterial(l2, par2 + 1, i3).isOpaque())
            // {
            // if (par4 <= 0)
            // {
