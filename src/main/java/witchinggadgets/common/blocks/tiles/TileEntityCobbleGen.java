...
public class TileEntityCobbleGen extends TileEntityWGBase
{
	...
	private void createCobble()
	{
		...
		if (cobble != null && WGConfig.allowdropsfrommachinery)
			...
			{
				EntityItem ei = new EntityItem(this.worldObj, this.xCoord + 0.5D + this.facing.offsetX * 0.66D, this.yCoord + 0.4D + this.facing.getOpposite().offsetY, this.zCoord + 0.5D + this.facing.offsetZ * 0.66D, cobble.copy());
