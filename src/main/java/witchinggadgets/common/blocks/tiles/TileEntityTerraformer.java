...
public class TileEntityTerraformer extends TileEntityWGBase implements IAspectContainer, IEssentiaTransport {
    ...
    public void updateEntity() {
        tick += 1;
        if (worldObj.getBlock(xCoord, yCoord + 1, zCoord) instanceof ITerraformFocus
                || worldObj.getTileEntity(xCoord, yCoord + 1, zCoord) instanceof ITerraformFocus) {
            ...
            else focus = (ITerraformFocus) worldObj.getBlock(xCoord, yCoord + 1, zCoord);
            ...
            if (!worldObj.isRemote && this.currentSuction != null) {
                if (drawEssentia()) this.addToContainer(this.currentSuction, 1);
                ...
                if (tick >= 20 && this.essentia.getAmount(currentSuction) >= 4) {
                    ...
                    if (worldObj.getBiomeGenForCoords(x, z).biomeID != transformBiome.biomeID
        ...
        } else if (this.getSuctionType(null) != null) this.setSuction(null, 0);
        if (tick >= 20) tick = 0;
    }
