...
// implements IAspectContainer
{
    ...
    public boolean renderStatic(Vector3 pos, int pass) {
        ...
        // }
        // }
        if (aspectFilter != null) tes.setColorOpaque_I(aspectFilter.getColor());
    ...
    public void save(NBTTagCompound tag) {
        super.save(tag);
        if (this.aspectFilter != null) tag.setString("AspectFilter", this.aspectFilter.getTag());
    }
    ...
    public Iterable<ItemStack> getDrops() {
        ...
        for (ItemStack is : super.getDrops()) drops.add(is);
        if (aspectFilter != null) drops.add(new ItemStack(ConfigItems.itemResource, 1, 13));
        return drops;
    }
    ...
    public boolean activate(EntityPlayer player, MovingObjectPosition hit, ItemStack stack) {
        ...
        } else if (player.getHeldItem() != null && player.getHeldItem().getItem().equals(ConfigItems.itemResource)
                && player.getHeldItem().getItemDamage() == 13) {
                    if (((IEssentiaContainerItem) player.getHeldItem().getItem()).getAspects(player.getHeldItem())
                            != null) {
                        aspectFilter = ((IEssentiaContainerItem) player.getHeldItem().getItem())
