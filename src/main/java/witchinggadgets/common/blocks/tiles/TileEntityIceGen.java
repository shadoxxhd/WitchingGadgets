...
public class TileEntityIceGen extends TileEntityWGBase
{
	...
	private void createIce()
	{
		...
		if (inventory instanceof IInventory)
			ice = InventoryUtils.placeItemStackIntoInventory(ice, (IInventory)inventory, this.facing.getOpposite().ordinal(), true);

		if (ice != null && WGConfig.allowdropsfrommachinery)
			if(facing.equals(ForgeDirection.UP)||facing.equals(ForgeDirection.DOWN))
			{
				EntityItem ei = new EntityItem(this.worldObj, this.xCoord + 0.5D, this.yCoord + 0.5 + this.facing.offsetY * 0.66D, this.zCoord + 0.5D, ice.copy());
			...
			{
				EntityItem ei = new EntityItem(this.worldObj, this.xCoord + 0.5D + this.facing.offsetX * 0.66D, this.yCoord + 0.4D + this.facing.getOpposite().offsetY, this.zCoord + 0.5D + this.facing.offsetZ * 0.66D, ice.copy());
	...
	private boolean canOutput()
	{
		TileEntity inventory = this.worldObj.getTileEntity(this.xCoord + this.facing.offsetX, this.yCoord + this.facing.offsetY, this.zCoord + this.facing.offsetZ);
		if (inventory instanceof IInventory)
			return InventoryUtils.insertStack((IInventory) inventory, new ItemStack(Blocks.ice), this.facing.getOpposite().ordinal(), false) == null;
		return true;
	}
