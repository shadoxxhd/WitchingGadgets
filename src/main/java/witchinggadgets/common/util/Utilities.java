...
public class Utilities {
    ...
    public static boolean consumeVisFromInventoryWithoutDiscount(EntityPlayer player, AspectList cost) {
        ...
        for (int b = 0; b < 4; b++) {
            if (baubles.getStackInSlot(b) != null && baubles.getStackInSlot(b).getItem() instanceof ItemAmuletVis) {
                boolean comnsumed = ((ItemAmuletVis) baubles.getStackInSlot(b).getItem())
                        .consumeAllVis(baubles.getStackInSlot(b), player, cost, true, true);
                if (comnsumed) return true;
            }
        }
    ...
    public static boolean stacksMatchWithOreDic(ItemStack stack0, ItemStack stack1) {
        for (int oid0 : OreDictionary.getOreIDs(stack0))
            for (int oid1 : OreDictionary.getOreIDs(stack1)) if (oid0 == oid1) return true;
        return false;
    }
    ...
    public static String getItemStackString(ItemStack stack) {
        ...
        if (stack.getItemDamage() == OreDictionary.WILDCARD_VALUE) result.append(":*");
        else if (stack.getItemDamage() > 0) result.append(':').append(stack.getItemDamage());
    ...
    public static boolean isDye(ItemStack item) {
        ...
        for (String d : dyes) if (Utilities.compareToOreName(item, d)) return true;
        return false;
    }

    public static int getDamageForDye(ItemStack item) {
        for (int d = 0; d < dyes.length; d++) if (Utilities.compareToOreName(item, dyes[d])) return d;
        return -1;
    }
    ...
    public static void extendPotionArray(int extendBy) {
        ...
        for (int i = 0; i < Potion.potionTypes.length; i++) potions[i] = Potion.potionTypes[i];
        try {
            Field field = null;
    ...
    public static boolean isRightMaterial(Material mat, Material[] materials) {
        for (Material m : materials) if (m == mat) return true;
        return false;
    }

    public static ItemStack getPickedBlock(World world, int x, int y, int z) {
        if (world.getBlock(x, y, z) == null) return null;
