    ...
    public static void handleWorldInteraction(PlayerInteractEvent event) {
        if (event.entityPlayer.getCurrentEquippedItem() != null
                && Block.getBlockFromItem(
                                event.entityPlayer.getCurrentEquippedItem().getItem())
            ...
            if (event.action == PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK
                ...
                TMultiPart part = meta == 1
                        ? new MultipartEssentiaTube_Valve(meta)
                        : meta == 3
                                ? new MultipartEssentiaTube_Filtered(meta)
                                : meta == 4 ? new MultipartEssentiaBuffer(meta) : new MultipartEssentiaTube(meta);
