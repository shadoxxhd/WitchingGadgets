        ...
        if (player != null && event.phase.equals(TickEvent.Phase.START)) {
            ...
            if (TileEntitySaunaStove.targetedPlayers.containsKey(player.getEntityId())) {
                ...
                for (AxisAlignedBB aabb : stove.boundingBoxes)
                    if (world.getEntitiesWithinAABB(EntityPlayer.class, aabb).contains(player)) flag = true;

                if (flag && !stove.isInvalid() && stove.tick > 0) {
                    if (world.rand.nextInt(100) == 0) {
                        WGModCompat.enviromineDoSaunaStuff(player, .01f, .01f);
                        if (player.getFoodStats().getFoodLevel() > 6) player.getFoodStats().addStats(-1, .1f);
                    }

                    if (world.isRemote && world.rand.nextInt(3) == 0) WitchingGadgets.proxy.createSweatFx(player);
                    ...
                    if (world.rand.nextInt(200) == 0) Thaumcraft.addWarpToPlayer(player, -1, true);
                } else TileEntitySaunaStove.targetedPlayers.remove(player.getEntityId());
            }
            if (!player.worldObj.isRemote && player.riddenByEntity != null
                    && player.riddenByEntity instanceof EntityLivingBase
                    && EnchantmentHelper
                            .getEnchantmentLevel(WGContent.enc_rideProtect.effectId, player.getCurrentArmor(3)) > 0) {
                player.riddenByEntity.attackEntityFrom(DamageSource.causePlayerDamage(player), 1);
                player.riddenByEntity
                        .addVelocity(player.getRNG().nextFloat() * .4f, .1f, player.getRNG().nextFloat() * .4f);
            ...
            if (Utilities.isPlayerUsingBow(player) && baubles != null
                    && (OreDictionary.itemMatches(
                            new ItemStack(WGContent.ItemMagicalBaubles, 1, 5),
                            baubles.getStackInSlot(1),
                            true)
                            || OreDictionary.itemMatches(
                                    new ItemStack(WGContent.ItemMagicalBaubles, 1, 5),
                                    baubles.getStackInSlot(2),
                                    true)))
                if (!Utilities.livingHasAttributeMod(
                        player,
                        SharedMonsterAttributes.movementSpeed,
                        new UUID(Lib.ATTRIBUTE_MOD_UUID, 6)))
                    Utilities.addAttributeModToLivingUnsaved(
                            player,
                            SharedMonsterAttributes.movementSpeed,
                            new UUID(Lib.ATTRIBUTE_MOD_UUID, 6),
                            "WGBowBonus",
