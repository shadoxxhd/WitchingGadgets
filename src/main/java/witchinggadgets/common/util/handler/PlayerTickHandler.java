        ...
        if (player != null && event.phase.equals(TickEvent.Phase.START)) {
            ...
            if (!player.worldObj.isRemote
                    ...
                    && EnchantmentHelper.getEnchantmentLevel(
                                    WGContent.enc_rideProtect.effectId, player.getCurrentArmor(3))
                            > 0) {
                player.riddenByEntity.attackEntityFrom(DamageSource.causePlayerDamage(player), 1);
                player.riddenByEntity.addVelocity(
                        player.getRNG().nextFloat() * .4f, .1f, player.getRNG().nextFloat() * .4f);
            ...
            if (Utilities.isPlayerUsingBow(player)
                    && baubles != null
                    && (OreDictionary.itemMatches(
                                    new ItemStack(WGContent.ItemMagicalBaubles, 1, 5), baubles.getStackInSlot(1), true)
                            || OreDictionary.itemMatches(
                                    new ItemStack(WGContent.ItemMagicalBaubles, 1, 5),
                                    baubles.getStackInSlot(2),
                                    true)))
                if (!Utilities.livingHasAttributeMod(
                        player, SharedMonsterAttributes.movementSpeed, new UUID(Lib.ATTRIBUTE_MOD_UUID, 6)))
