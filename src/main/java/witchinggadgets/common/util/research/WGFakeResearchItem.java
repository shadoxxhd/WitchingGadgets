...
public class WGFakeResearchItem extends ResearchItem {
    ...
    public WGFakeResearchItem(String key, String category, String original, String originalCategory, int displayX,
            int displayY, ResourceLocation icon) {
        super(key, category, new AspectList(), displayX, displayY, 1, icon);
        this.original = ((ResearchCategoryList) ResearchCategories.researchCategories.get(originalCategory)).research
                .get(original);
        this.setupOriginal();
    }

    public WGFakeResearchItem(String key, String category, String original, String originalCategory, int displayX,
            int displayY, ItemStack icon) {
        super(key, category, new AspectList(), displayX, displayY, 1, icon);
        this.original = ((ResearchCategoryList) ResearchCategories.researchCategories.get(originalCategory)).research
                .get(original);
        this.setupOriginal();
    ...
    private void setupOriginal() {
        if (this.original.siblings == null) this.original.setSiblings(new String[] { this.key });
        else {
